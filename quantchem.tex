...
\[\psi_e = \Phi_0,\]
что по сути представляет собой метод Хартри-Фока, или же метод самосогласованного поля. Отсюда мы в дальнейшем получим набор \(\{\psi_p\}\).

Если \(\psi_e = \sum_\mu \varepsilon c_\mu \Phi_\mu + \cancel{\dots},\)
то метод будет называться многоконфигурационным методом самосогласованного поля.
...
\begin{wrapfigure}{r}{0.5\textwidth}
    \centering
    \includegraphics[width=0.3\textwidth]{images/orbitalsH.png}
...
\[\varphi = \chi_1 + \chi_2,\]
в данном случае это антисвязывающая, или разрыхляющая орбиталь. Таким образом в общем случае мы можем каждому иону сопоставить атомную орбиталь \(\chi_\mu\) (АО) и представить общую пространственную орбиталь как суперпозицию данных АО:
\begin{equation}
...
Введём определение оператора Фока: 
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:hf_fock}
\end{equation}
что называется \textit{уравнениями Хартри-Фока}. Данные уравнения решаются итеративно, пока не будет достигнуто согласование между волновыми функциями. Поэтому данный метод ещё называется \text{методом самосогласованного поля}. 
Оператор Фока можно так же представлять в виде 
\begin{equation}
    \hat{f} = \hat{h} + \sum_{j=1}^{N}\hat{J}_j - \sum_{j=1}^{N}\hat{K}_j,
\end{equation}
где, обменный оператор \(\hat{K}_j\) 
\begin{equation}
    \hat{K}_j \psi_j= \int_{[2]} d2 \psi_j^* (2) \dfrac{1}{r_{12}} \psi_j (2) \psi_j (1),
\end{equation}
является интегральным оператором. Поэтому уравнения Хартри-Фока так же являются интегро-дифференциальными. Стандартным методом решения таких уравнений является \href{https://en.wikipedia.org/wiki/DIIS}{DIIS}. Данный метод позволяет сойтись методу ХФ даже в самых патологических случаях. Однако такой метод требует содержать много фоковских матриц в памяти, что бывает очень затратно. Был придуман более усовершенствованный метод в 2015 году под названием \href{https://pubs.acs.org/doi/10.1021/ct501114q}{CROP}.

Остается вопрос начального приближения. В качестве него можно выбрать, например, следующее

\begin{equation}
    \hat{f}_n = \sum_{ab} A_{ab} a^{\dagger}_a a_b,
\end{equation}
где a, b - индексы орбитали, \(A_{ab} = \dfrac{I_a + I_b}{2}\) - средний потенциал ионизации для каждой орбитали.

Годится так же и одноэлектронная часть гамильтониана в качестве начального приближения:

\begin{equation}
    \hat{f}_n = \hat{h}
\end{equation}

И одним из самых лучших является суперпозиция атомных плотностей SAD, основанное на матрицах плотности.

\subsection{Диагонализация уравнений Хартри-Фока}

В форме \eqref{eq:hf_fock} уравнения ХФ являдются довольно неудобными, хотелось бы иметь возможность привести их к виду:

\begin{equation}
    \hat{f} \tilde{\psi}_i (1) = \tilde{\varepsilon}_i \tilde{\psi}_i (1),
\end{equation}
с помощью унитарного преобразования, однако сам оператор Фока может оказаться неинвариантным относительно таких преобразований. Проверим это на примере кулоновского оператора. Пусть \(\psi_j' = \sum U_{jk}\psi_k\), тогда 

\begin{multline}
    \hat{J}_j' \psi_i = \int_{[2]} d2 \dfrac{1}{|\bm{r}|} \left(\sum_k U_{jk} \psi_k (2)\right)^* \left(\sum_l U_{jl} \psi_l (2)\right) = \int_{[2]} \dfrac{d2}{|\bm{r}|} \left(\sum_{kl} \underbrace{U_{jk}^*}_{=U_{jk}^{\dagger}} U_{jl} \psi_k^* \psi_l(2) \right) \psi_i (1) =\\=
    \int_{[2]} \dfrac{d2}{|\bm{r}|} \delta_{kl} \psi_k^* (2) \psi_l (2) = \int_{[2]} \dfrac{d2 \psi_k^*(2) \psi_k(2)}{|\bm{r}|} \psi_i(1) = \hat{J}_k \psi_i (1),
\end{multline}
то есть оператор \(\hat{J}\) действительно является унитарным относительно унитарного преобразования. Аналогично, можно показать инвариантность для обменного оператора \(\hat{K}\).
