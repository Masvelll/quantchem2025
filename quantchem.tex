...
%     xlabel style = {below=30pt},    % сдвигаем метку оси X вниз
%     ylabel style = {left=20pt},     % сдвигаем метку оси Y влево
%     % ticklabel style = {font=\small},
%     line width = 1.5pt,
%     every axis x label/.style={at={(ticklabel* cs:1.0)}, anchor=north, xshift=20pt},
%     every axis y label/.style={at={(ticklabel* cs:1.0)}, anchor=east, yshift=15pt}
    
% ]
% \addplot[black, thick] {1 + abs(x)/2 - (x-0.5)^2 + exp(-(x-1.5)^2) / 1.5};
% \node[black, right] at (axis cs: 1.0, 1.5) {$\psi(r_{12})$};
% \end{axis}
% \end{tikzpicture}
